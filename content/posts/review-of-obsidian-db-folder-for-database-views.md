...
Use Obsidian DB Folder for operational hubs where you actively manage states, assignees, and deadlines—such as an active sprint board or an editorial calendar. Use Dataview for high-level, read-only [dashboards](/posts/advanced-dataview-js-scripts-for-custom-obsidian-dashboards/) that aggregate information across the entire vault, such as a [weekly review](/posts/obsidian-template-for-weekly-reflection-and-planning/) page that pulls in all notes modified in the last seven days regardless of their folder location.
