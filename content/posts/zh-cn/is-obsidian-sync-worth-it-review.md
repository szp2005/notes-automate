...
Syncthing 是一个免费、开源的端到端同步工具，许多高级用户对此赞不绝口。它运行良好，真正安全（TLS + 设备验证），并且根本没有云端中介。问题在于两台设备必须同时在线才能同步，或者你需要运行一个中继/始终在线的设备来桥接它们。即使对于经验丰富的用户，正确设置 Syncthing 也需要 30-90 分钟，并且需要偶尔的维护 —— 防火墙规则、应用更新、冲突解决。
...
如果你想进一步支持 Obsidian 开发者，Obsidian Catalyst 许可证 是一次性购买的服务，旨在资助开发并授权你访问内部构建版本 —— 它不能替代 Sync，但值得了解。
...
**注重安全的用户。** 如果你的 vault 包含任何你认为敏感的内容 —— 心理咨询笔记、客户工作资料、医疗信息、个人财务信息 —— E2EE 就不是可选项，而是底线。Obsidian Sync 和 Sync.com（用于辅助文件存储）是少数几家在消费者层面认真对待此问题的服务提供商之一。说到这里：如果你足够重视 E2EE 并愿意为你的笔记买单，那么使用像 NordVPN 这样值得信赖的 VPN 将这种保护扩展到你的整个网络连接是非常值得的。
...
**单设备用户。** 如果你只在一台机器上写作，并且永远不需要在其他地方查看笔记，那么你根本不需要任何同步解决方案。备份到外部硬盘或 Sync.com 可以以更低的成本满足你的数据安全需求。
...
**准备好尝试了吗？** 开始你的 Obsidian Sync 免费试用 —— 免费使用 2 个月，无任何承诺。如果你觉得不合适，你的笔记仍然属于你，你可以零摩擦地切换到任何替代方案。

*如果你重视笔记的 E2EE，请考虑将这种保护扩展到你更广泛的数字生活。Sync.com 为其他所有内容提供零知识加密的云存储，而 NordVPN 保护你在最佳工作状态下所连接的网络。*
...
- [核心困境：付费的便利性与免费的控制权](/posts/obsidian-sync-vs-syncthing-for-free-note-synchronization/)
