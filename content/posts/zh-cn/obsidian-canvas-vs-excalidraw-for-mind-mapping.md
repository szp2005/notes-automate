...
| **实时协作** | 无 | 通过 Excalidraw.com / Excalidraw+ |
...
Canvas 没有任何协作功能。而 Excalidraw 文件可以直接在 Excalidraw.com 上打开并进行实时共享。对于需要强大共享工作区、持久协作房间和端到端加密共享的高级用户和团队，Excalidraw+ 提供了专为此工作流打造的高级服务。如果你是项目经理或教育工作者，需要与不使用 Obsidian 的人共享图表，这是一个决定性的优势。
...
**记录流程的项目经理：**
Excalidraw 胜出。构建带有合适的连接器、形状样式和标记决策点的泳道图或流程图。导出为 SVG，粘贴到共享文档中，或通过 Excalidraw+ 与你的团队共享——所有这些都不需要你的协作者接触 Obsidian。
...
如果有疑问，安装 Excalidraw 插件并保持启用 Canvas。使用 Canvas 作为你的知识组织层，使用 Excalidraw 作为你的绘图工具。没有规定说不能两者兼得。
...
为了在你的所有设备上无缝保护和访问视觉地图，请考虑通过使用 Obsidian Sync 来支持开发者——它无需任何特殊配置即可处理 `.canvas` 和 `.excalidraw` 文件。

如果你想更深入地构建智能集成这两种工具的结构化第二大脑，Udemy 上这门备受好评的 PKM 课程详细且实用涵盖了视觉工作流设计、链接策略和 Zettelkasten 的实施。
