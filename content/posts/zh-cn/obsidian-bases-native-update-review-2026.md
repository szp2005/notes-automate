...
> **快速总结：** 2026年的 Obsidian Bases 原生更新彻底改变了该插件处理[元数据](/zh-cn/posts/explaining-obsidian-properties-for-advanced-metadata-schemas/)的方式，从缓慢的 Markdown 包装器转向了与核心集成的 SQLite 后端。它提供了无缝的类似 Notion 的表格、看板和画廊，且没有任何明显的延迟，使其成为寻求结构化数据同时避免供应商锁定的 Obsidian 重度用户的终极数据库解决方案。
